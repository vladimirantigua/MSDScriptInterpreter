...
public:
    ...
    virtual PTR(Val) interp() = 0; // changed to virtual int interp() = 0;
    virtual bool has_variable() = 0;
    
    virtual PTR(Expr) subst(std::string sub, PTR(Expr) expr) = 0;
    //    Add a print method to Expr:
...
public:
    ...
    PTR(Expr) subst(std::string sub, PTR(Expr) expr);
